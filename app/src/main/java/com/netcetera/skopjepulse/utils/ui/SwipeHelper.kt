package com.netcetera.skopjepulse.utils.ui
...
/**
 *Abstract class for drawing the background and the button on swipe to delete
 */
abstract class SwipeHelper(
  ...
  class UnderlayButton(
    ...
    init {
      ...
      intrinsicWidth = titleBounds.width() + 2 * horizontalPadding + 25
    }
    ...
    fun draw(canvas: Canvas, rect: RectF) {
      ...
      val drawable = ContextCompat.getDrawable(context, R.drawable.ic_swipe_to_delete)
      val bitmap = drawable?.toBitmap()
      val y = (rect.height() / 2) - (bitmap!!.height / 2)
      val x = (rect.width() /2) - (bitmap.width / 2)


      canvas.drawBitmap(bitmap, rect.left + x, rect.top + y, paint)
